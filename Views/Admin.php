<div style="padding-left: 50px; padding-right: 50px;">
			...
			<div align="left" style="margin-bottom: 5px;">
				...
				<br />
				<a href="/admin/?do=logout">Выход</a>
			</div>
...
$(document).ready(function(){
	...
	jQuery(document).on("click", ".edit", function() {
		...
		$.ajax({
			...
			success: function(data) {
				...
				jQuery("#name_user").val(data.name_user);
				jQuery("#text_email").val(data.text_email);
				jQuery("#text_description").val(data.text_description);
				jQuery("#action").val("update");
				jQuery("#button_action").val("Обновить");
				jQuery(".modal-title").text("Редактировать запись");
				jQuery("#apicrudModal").modal("show");
			}
		}
		);
	}
	...
	jQuery(document).on("click", ".backcomplete", function() {
		var id = jQuery(this).attr("id");
		var action = "backcomplete";
			$.ajax({
				url: "/task/action.php",
				method: "POST",
				data: {id: id, action: action},
